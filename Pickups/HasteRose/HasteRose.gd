class_name HasteRose extends Area2D
...
func _on_body_entered(body):
	if body is Player:
		body.haste_rose_picked_up.emit()
		...
		fade_tween.parallel().tween_property($HasteRoseGlimmer, "volume_db", -35, sound_fade_time_seconds)
